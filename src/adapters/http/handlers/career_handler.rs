...
pub async fn get_careers(State(state): State<AppState>) -> Result<Json<Vec<Career>>, StatusCode> {
...
pub async fn get_career_by_id(
    ...
    Path(id): Path<i32>,
) -> Result<Json<Career>, StatusCode> {
    let career = sqlx::query_as::<_, Career>("SELECT * FROM career WHERE id = $1")
        .bind(&id)
...
) -> Result<(StatusCode, Json<Career>), StatusCode> {
...
pub async fn update_career(
    ...
    let result = sqlx::query(
        "UPDATE career SET name = COALESCE($1, name), is_active = COALESCE($2, is_active) WHERE id = $3",
    )
    .bind(&payload.name)
