...
pub async fn get_courses(State(state): State<AppState>) -> Result<Json<Vec<Course>>, StatusCode> {
...
pub async fn update_course(
    ...
    let result = sqlx::query(
        "UPDATE course SET name = COALESCE($1, name), is_active = COALESCE($2, is_active), credits = COALESCE($3, credits), career_id = COALESCE($4, career_id) WHERE id = $5",
    )
    .bind(&payload.name)
